...
$behaviorWorld = getPackList($projectRoot . '/world_behavior_packs.json');
$resourceWorld = getPackList($projectRoot . '/world_resource_packs.json');

$behaviorMap = scanManifests($projectRoot . '/behavior_packs');
$resourceMap = scanManifests($projectRoot . '/resource_packs');
...
foreach (glob($projectRoot . '/world_*_packs.json') as $jsonFile) {
    @mkdir($exportDir, 0777, true);
    if (copy($jsonFile, $exportDir . '/' . basename($jsonFile))) $totalFiles++;
}

echo $totalFiles > 0
    ? "Hoan tat! Da copy $totalFiles file sang $exportDir\n"
    : "Chua copy duoc file nao! Kiem tra folder va JSON world\n";
